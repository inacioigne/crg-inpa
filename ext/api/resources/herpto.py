...
from .peixes import filterLocal
...
def filterDate(q, args):
    ...
    if fromDate and toDate:
        q = q.filter(Herpto.data_coleta >= fromDate, Herpto.data_coleta <= toDate ) 
        return q
    elif fromDate:
        q = q.filter(Herpto.data_coleta >= fromDate)
        return q
    elif toDate:
        q = q.filter(Herpto.data_coleta <= toDate)
        return q
    return q

class Collection_Herpto(Resource):
    def get(self):
        args = request.args
...
class HerptoByCodigo(Resource):
    def get(self, codigo):
        q = session.query(Herpto).filter(Herpto.codigo == codigo).first()
